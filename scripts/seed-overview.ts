import { PrismaClient, MeetingType, ProjectStatus } from "../app/generated/prisma/client.js";
import { Pool } from "pg";
...
const connectionString = process.env.DATABASE_URL || "postgresql://postgres:postgres@localhost:5433/faculty_bugget";
...
function parseThaiDate(dateStr: string): Date | null {
  ...
  const thaiMonths: Record<string, number> = {
    ...
    "พฤษภาคม": 4, "มิถุนายน": 5, "กรกฏคม": 6, "กรกฎาคม": 6,
    "สิงหาคม": 7, "กันยายน": 8, "ตุลาคม": 9, "พฤศจิิกายน": 10,
    ...
    try {
      ...
      const project = await prisma.project.create({
        data: {
          ...
          participantCount: item.participantCount ? parseInt(item.participantCount) : null,
      ...
      errorCount++;
      console.error(`❌ Failed to import ${item.projectCode || item.id}:`, error);
    }
  }
